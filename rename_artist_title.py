...
def rename_artist_title(directory):
    ...
    for mp3_file in mp3_files:
        try:
            ...
            # Check if both artist and title exist
            if audiofile.tag.artist and audiofile.tag.title:
                # Create new filename from artist and title
                new_name = sanitize_filename(f"{audiofile.tag.artist} - {audiofile.tag.title}.mp3")
                ...
                print(f"Skipping {mp3_file.name}: Missing artist or title tag")
                skipped_count += 1
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python rename_artist_title.py <directory>")
        sys.exit(1)
    
    directory = sys.argv[1]
    rename_artist_title(directory) 